...
if [ "$TILES_C" == "0" ]; then
    ...
    if [ "$extension" == "bz2" ]; then
        bzcat ${OSM_DUMP} | java -Xmx4G -Djava.io.tmpdir=/opt/tmp -jar ${GAZETTEER_JAR} --threads 2 split
    elif [ "$extension" == "osm" ]; then
        java -Xmx4G -Djava.io.tmpdir=/opt/tmp -jar ${GAZETTEER_JAR} --threads 2split ${OSM_DUMP}
    elif [ "$extension" == "pbf" ]; then
        osmconvert ${OSM_DUMP} | java -Xmx4G -Djava.io.tmpdir=/opt/tmp -jar ${GAZETTEER_JAR} --threads 2 split
    else 
        echo "Unrecognized osm dump file extension $extension"
    fi
	
	
    java -Xmx4G -Djava.io.tmpdir=/opt/tmp -jar ${GAZETTEER_JAR} --threads 2 tile-buildings --out-dir ${OSM_TILES}
